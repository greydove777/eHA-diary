...
export default function HeadacheDiary() {
  ...
  const [view, setView] = useState("calendar"); // calendar | entry | stats
    ...
    <div>
      ...
      <div style={{ ...S.card, padding: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 3 }}>
          ...
          {Array.from({ length: totalDays }, (_, i) => i + 1).map(day => {
            ...
            const bgColor = has
              ? mp <= 3 ? "rgba(42,154,106,0.15)" : mp <= 6 ? "rgba(224,144,32,0.15)" : "rgba(208,64,64,0.15)"
              : "rgba(255,255,255,0.6)";
            const borderColor = isSelected ? C.primary : has
              ? mp <= 3 ? C.green : mp <= 6 ? C.orange : C.red
              : "rgba(74,138,176,0.2)";

            return (
              <div key={day} onClick={() => { setSelectedDay(day); setView("entry"); }}
                style={{ textAlign: "center", padding: "6px 2px", borderRadius: 10, background: bgColor, border: `2px solid ${borderColor}`, cursor: "pointer", position: "relative" }}>
        ...
        <div style={{ display: "flex", gap: 12, marginTop: 12, justifyContent: "center", flexWrap: "wrap" }}>
          {[["輕微 (1-3)", C.green], ["中等 (4-6)", C.orange], ["嚴重 (7-10)", C.red]].map(([l, c]) => (
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
        {[
          ...
          <div key={label} style={{ flex: 1, background: C.card, border: `1px solid ${C.cardBorder}`, borderRadius: 12, padding: "14px 8px", textAlign: "center", boxShadow: "0 2px 8px rgba(74,154,191,0.07)" }}>
            <div style={{ fontSize: 24, fontWeight: "bold", color }}>{val}</div>
            <div style={{ fontSize: 10, color: C.textLight, marginTop: 4 }}>{label}</div>
          </div>
        ))}
      </div>
      ...
      <div>
        ...
        <div style={S.card}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 11, color: C.textLight }}>1最輕微 ～ 10最嚴重（0 = 無痛）</span>
          </div>
          {[["morning", "早上"], ["afternoon", "下午"], ["evening", "晚上"]].map(([field, label]) => (
            <div key={field} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
              <div style={{ width: 40, fontSize: 13, color: C.textMid, flexShrink: 0 }}>{label}</div>
        ...
        <div style={S.card}>
          ...
          <div>
            {SYMPTOMS.map(s => (
              <button key={s} style={S.pill((e.symptoms || []).includes(s))} onClick={() => toggleSymptom(s)}>{s}</button>
            ))}
          </div>
        </div>
        ...
        <div style={S.card}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap" }}>
              {MEDICATIONS.map(m => (
                <button key={m} style={S.pill(e.medication === m)} onClick={() => updateEntry("medication", m)}>{m}</button>
              ))}
            </div>
          </div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button style={{ ...S.backBtn, flex: 1 }} disabled={selectedDay <= 1}
            onClick={() => setSelectedDay(d => d - 1)}>
            ‹ 前一天
          </button>
          <button style={{ ...S.primaryBtn, flex: 1 }} disabled={selectedDay >= totalDays}
            onClick={() => setSelectedDay(d => d + 1)}>
            後一天 ›
          </button>
        </div>
...
<head>
<meta charset="UTF-8"/>
<title>頭痛日記 ${year}年${month}月</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: 'Microsoft JhengHei','PingFang TC','Noto Sans TC',sans-serif; font-size: 11px; color: #1a3a52; background: #fff; }
  ...
  .title { font-size: 20px; font-weight: bold; color: #2a82aa; }
  .subtitle { font-size: 11px; color: #7aaac8; margin-top: 2px; }
  ...
  tr:hover td { background: #e0f0fa; }
  .day-col { font-weight: bold; color: #2a82aa; background: #e8f5fb !important; width: 28px; }
  .pain-0 { color: #aaa; }
  .pain-low { color: #2a9a6a; font-weight: bold; }
  .pain-mid { color: #e09020; font-weight: bold; }
  .pain-high { color: #d04040; font-weight: bold; }
  .check { color: #2a82aa; }
  .footer { margin-top: 8px; font-size: 9px; color: #7aaac8; text-align: center; border-top: 1px solid #c0d8e8; padding-top: 6px; }
  .summary { display: flex; gap: 16px; margin-bottom: 8px; }
  .stat { background: #e8f5fb; border: 1px solid #b0d8f0; border-radius: 6px; padding: 6px 14px; text-align: center; }
  .stat-val { font-size: 18px; font-weight: bold; color: #2a82aa; }
  .stat-lbl { font-size: 9px; color: #7aaac8; }
</style>
</head>
<body>
<div class="page">
  <div class="header">
    <div>
      <div class="title">🧠 頭痛日記</div>
      <div class="subtitle">內容根據台灣頭痛學會之建議制定</div>
    </div>
    <div class="meta">
      <div><b>${year} 年 ${month} 月</b></div>
      ${nextVisit ? `<div>下次回診：${nextVisit}</div>` : ""}
  ...
  <table>
    ...
    <tbody>
      ${rows.map(({ day, morning, afternoon, evening, symptoms, duration, medication, medicationEffect, stress, exercise }) => {
        const painClass = (v) => {
          const n = parseInt(v);
          if (!n) return "pain-0";
          if (n <= 3) return "pain-low";
          if (n <= 6) return "pain-mid";
          return "pain-high";
        };
        const sym = symptoms || [];
        const hasMed = medication && medication !== "未服藥";
        return `<tr>
          <td class="day-col">${day}</td>
          <td class="${painClass(morning)}">${morning || "0"}</td>
          <td class="${painClass(afternoon)}">${afternoon || "0"}</td>
          <td class="${painClass(evening)}">${evening || "0"}</td>
          ...
          <td>${duration || ""}</td>
          <td style="font-size:9px">${hasMed ? medication : ""}</td>
  ...
  <div class="footer">
    僅供個人記錄使用，不可取代求醫之需要，亦不能作為自我診斷或選擇治療的依據。關於個人疾病治療計畫請向您的醫療團隊諮詢專業建議。
  </div>
</div>
</body>
</html>`;
  ...
  const StatsView = () => (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <div style={{ fontSize: 18, fontWeight: "bold", color: C.primary }}>
          {year} 年 {month} 月 統計
        </div>
        <button onClick={exportPDF}
          style={{ background: "linear-gradient(135deg,#3aaa7a,#2a8a5a)", border: "none", borderRadius: 8, color: "#fff", padding: "9px 16px", fontSize: 13, fontFamily: FF, cursor: "pointer", boxShadow: "0 2px 8px rgba(42,150,100,0.3)", display: "flex", alignItems: "center", gap: 6 }}>
          🖨️ 匯出PDF
        </button>
      </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
        {[
          ...
          <div key={label} style={{ flex: 1, background: C.card, border: `1px solid ${C.cardBorder}`, borderRadius: 12, padding: "16px 8px", textAlign: "center", boxShadow: "0 2px 8px rgba(74,154,191,0.07)" }}>
            <div style={{ fontSize: 26, fontWeight: "bold", color }}>{val}</div>
            <div style={{ fontSize: 10, color: C.textLight, marginTop: 4 }}>{label}</div>
          </div>
        ))}
      </div>
      ...
      {/* 每日明細 */}
      {Object.entries(monthData).filter(([, e]) => e.morning || e.afternoon || e.evening).sort((a, b) => parseInt(a[0]) - parseInt(b[0])).length > 0 && (
        <div style={S.card}>
          ...
          {Object.entries(monthData).filter(([, e]) => e.morning || e.afternoon || e.evening).sort((a, b) => parseInt(a[0]) - parseInt(b[0])).map(([day, e]) => (
            <div key={day} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid rgba(74,138,176,0.08)", cursor: "pointer" }}
              ...
              <div style={{ width: 36, height: 36, borderRadius: "50%", background: painColor(maxPain(parseInt(day))) + "20", border: `2px solid ${painColor(maxPain(parseInt(day)))}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: "bold", color: painColor(maxPain(parseInt(day))), flexShrink: 0 }}>
                {day}
              </div>
              <div style={{ flex: 1 }}>
      ...
      {headacheDays === 0 && (
        <div style={{ ...S.card, textAlign: "center", padding: 32 }}>
          <p style={{ color: C.textLight, margin: 0 }}>本月尚無頭痛紀錄。</p>
        </div>
      )}
    </div>
  );
